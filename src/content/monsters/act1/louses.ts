...
// Order (LouseNormal.java:36-55 / LouseDefensive.java): the constructor rolls
// setHp and then D, so D lands in afterHpRoll right behind each louse's own HP
// roll (hp0,D0,hp1,D1); usePreBattleAction rolls CURL_UP for every louse after
// all of them are built (CURL0,CURL1).
...
function louseBiteRoll(ctx: EffectCtx, self: MonsterState): void {
  self.data.biteDamage = ctx.asc >= 2 ? ctx.rng("monsterHpRng").randomRange(6, 8) : ctx.rng("monsterHpRng").randomRange(5, 7);
}
